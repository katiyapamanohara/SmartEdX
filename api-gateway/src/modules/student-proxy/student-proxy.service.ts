...
export class StudentProxyService {
  private readonly logger = new Logger(StudentProxyService.name);
  ...
  async forwardRequest(
    ...
    const url = `${this.studentServiceUrl}/api/${path}`;
    ...
    try {
      ...
      if (error.response) {
        this.logger.error(
          `Student service error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
        );
