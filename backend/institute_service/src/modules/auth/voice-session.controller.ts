...
export class VoiceSessionController {
  ...
  private readonly activeSessions = new Map<
    string,
    { institute_id: string; started_at: Date; meta_data?: Record<string, any> }
  >();
  ...
  startSession(@Body() dto: StartVoiceSessionDto) {
    ...
    this.logger.log(
      `Voice session started: ${dto.sip_session_id} (institute: ${dto.institute_id}, type: ${dto.meta_data?.type ?? 'unknown'})`,
    );
  ...
  endSession(@Body() dto: EndVoiceSessionDto) {
    ...
    if (session) {
      ...
      this.logger.log(
        `Voice session ended: ${dto.session_id} (duration: ${duration}s, turns: ${dto.chat_history?.length ?? 0})`,
      );
      this.activeSessions.delete(dto.session_id);
    } else {
      this.logger.warn(
        `Voice session end received for unknown session: ${dto.session_id}`,
      );
    }
