...
export class PayhereService {
  ...
  buildCheckoutParams(data: {
    ...
    const hash = this.generateHash(merchantId, data.orderId, amountFormatted, data.currency);

    return {
      sandbox,
      merchant_id: merchantId,
      return_url: this.configService.get<string>('PAYHERE_RETURN_URL') || 'http://localhost:3000/dashboard/billing?status=success',
      cancel_url: this.configService.get<string>('PAYHERE_CANCEL_URL') || 'http://localhost:3000/dashboard/billing?status=cancel',
      notify_url: this.configService.get<string>('PAYHERE_NOTIFY_URL') || 'http://localhost:5002/payhere/notify',
      order_id: data.orderId,
      items: data.itemName,
  ...
  async handleNotify(body: Record<string, string>): Promise<{ success: boolean }> {
    const {
      merchant_id,
      order_id,
      payment_id,
    ...
    if (md5sig !== expectedSig) {
      this.logger.warn(`PayHere notify: invalid signature for order ${order_id}`);
    ...
    if (status_code === '2') {
      try {
        const subscription = await this.subscriptionRepo.findByOrderId(order_id);
        if (subscription) {
          subscription.payherePaymentId = payment_id;
          subscription.status = 'active';
          await this.subscriptionRepo.save(subscription);
          this.logger.log(`Subscription ${subscription.id} activated via PayHere order ${order_id}`);
        }
      } catch (err) {
        this.logger.error(`Failed to update subscription for order ${order_id}`, err);
      }
    }
