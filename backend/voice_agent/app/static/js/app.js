...
const languageOverrideSelect = document.getElementById("languageOverride");

function hasSelectedLanguage() {
  return Boolean(languageOverrideSelect && languageOverrideSelect.value);
}
...
function handleRunConfigChange() {
  if (!hasSelectedLanguage()) {
    ...
    addSystemMessage("Please select a language to start.");
    return;
  }
  ...
  if (websocket && websocket.readyState === WebSocket.OPEN) {
    ...
    addConsoleEntry(
      ...
      "Reconnecting due to settings change",
      {
        ...
        language: languageOverrideSelect.value,
...
languageOverrideSelect.addEventListener("change", handleRunConfigChange);
...
function getWebSocketUrl() {
  const protocol =
    ...
    window.location.hostname === "127.0.0.1"
      ...
      : "wss://";
  const baseUrl =
    protocol + window.location.host + "/ws/" + userId + "/" + sessionId;
  ...
  // Language is mandatory for this client flow.
  params.append("language", languageOverrideSelect.value);
...
function connectWebsocket() {
  if (!hasSelectedLanguage()) {
    ...
    addSystemMessage("Please select a language from the menu to connect.");
    return;
  }
  ...
  websocket.onmessage = function (event) {
    ...
    // Handle server-side disconnect signal
    if (adkEvent.type === "disconnect") {
      const reason = adkEvent.reason || "Unknown reason";
      addSystemMessage("Server disconnected: " + reason);
      addConsoleEntry(
        "error",
        "Server Disconnect",
        {
  ...
  websocket.onclose = function () {
    ...
    if (!hasSelectedLanguage()) {
      addSystemMessage("Disconnected. Select a language to reconnect.");
      return;
    }
...
if (hasSelectedLanguage()) {
  ...
  updateConnectionStatus(false);
  document.getElementById("sendButton").disabled = true;
  addSystemMessage("Select a language to start the conversation.");
}
...
function startAudio() {
  ...
  startAudioPlayerWorklet().then(([node, ctx]) => {
    ...
    if (!waveformVisualizer) {
      waveformVisualizer = new WaveformVisualizer("waveformCanvas", ctx);
      waveformVisualizer.connect(node);
