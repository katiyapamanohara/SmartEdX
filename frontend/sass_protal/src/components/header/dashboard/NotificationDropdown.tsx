...
function TypeIcon({ type }: { type: Notification["type"] }) {
  ...
  if (type === "cheat_alert") {
    return (
      <span className="flex items-center justify-center w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/40 shrink-0 animate-pulse">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-red-500" xmlns="http://www.w3.org/2000/svg">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" stroke="currentColor" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
        </svg>
      </span>
    );
  }
...
function getNotifUrl(notif: Notification): string {
  if (notif.instituteId) {
    if (notif.type === "cheat_alert") return `/dashboard/institute/${notif.instituteId}`;
    return `/dashboard/institute/${notif.instituteId}`;
  }
  return "/dashboard";
}
...
export default function NotificationDropdown() {
  ...
  const handleOpen = async () => {
    setIsOpen(true);
    if (unreadCount > 0) {
      const instituteIds = [...new Set(notifications.filter((n) => !n.isRead && n.instituteId).map((n) => n.instituteId as string))];
      if (instituteIds.length) {
        await notificationService.markAllAsRead(instituteIds);
  ...
  const handleNotifClick = async (notif: Notification) => {
    handleClose();
    if (!notif.isRead && notif.instituteId) {
      notificationService.markAsRead(notif.instituteId, notif.id);
  ...
  return (
    <div className="relative">
      ...
      <Dropdown
        isOpen={isOpen}
        onClose={handleClose}
        className="absolute left-0 lg:left-auto lg:right-0 mt-4.25 flex h-120 w-87.5 max-w-[calc(100vw-16px)] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark"
      >
        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          ...
          {!loading && notifications.length === 0 && (
            <li className="flex flex-col items-center justify-center py-10 text-sm text-gray-400 gap-2">
              <svg width="32" height="32" viewBox="0 0 20 20" fill="none" className="opacity-40" xmlns="http://www.w3.org/2000/svg">
