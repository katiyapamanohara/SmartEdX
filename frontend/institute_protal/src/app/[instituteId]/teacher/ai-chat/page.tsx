...
export default function TeacherAiChatPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/ai/chat-history/save`, {
      method: "POST",
  ...
  // ── Auto-select course from query params (only when no saved session) ─────
  useEffect(() => {
    if (!hydrated) return;
  ...
  async function handleSelectCourse(course: Course) {
    ...
    // Try to restore from Qdrant (cross-session history)
    const user = authService.getUser();
    if (user?.id && instituteId) {
      try {
        const res = await fetch(
          `${apiUrl}/api/ai/chat-history/load?institute_id=${encodeURIComponent(instituteId)}&user_id=${encodeURIComponent(user.id)}&course_id=${encodeURIComponent(course.id)}`,
        );
        if (res.ok) {
