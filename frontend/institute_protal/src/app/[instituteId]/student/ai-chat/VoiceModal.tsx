...
export default function VoiceModal({
  ...
  //    initial attempt fires without a valid user-activation context) ─────────
  useEffect(() => {
  ...
  async function openSystemPip() {
    ...
    try {
      ...
      // Inject minimal styles — all our PiP content uses inline styles,
      // but we need the keyframe for the spinner animation.
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ── Shared compact PiP body (orb + status + controls) ───────────────────
  // Used by both the in-browser floating widget and the system PiP portal.
  ...
  if (pipMode) {
    return (
      <>
        {/* Hidden capture elements must stay in the main document */}
        {hiddenMedia}
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        {step === "session" && (
          ...
          >
            {/* PiP icon: screen with inset window */}
            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
